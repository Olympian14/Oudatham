...
export default function Overview({ patients, currentIdx, onSelect, onNew, onDelete }: OverviewProps) {
  return (
    <div id="overview-component" className="space-y-6">
      ...
      {patients.length === 0 ? (
        <div className="bg-slate-50/60 dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-800/80 rounded-2xl p-16 text-center flex flex-col items-center space-y-4">
          ...
          <p className="text-xs text-slate-400 dark:text-slate-500 max-w-sm">Click "New Case" to begin a clinical consultation.</p>
        </div>
      ) : (
            ...
            return (
              <button key={p.id} onClick={() => onSelect(idx)} className={`group relative text-left p-5 rounded-xl border transition-all duration-150 ${isActive ? "bg-white dark:bg-slate-900 border-indigo-500 shadow-lg shadow-indigo-950/30 ring-1 ring-indigo-500/20" : "bg-slate-50/60 dark:bg-slate-900/60 border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 hover:bg-white dark:hover:bg-slate-900"}`}>
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-slate-500 dark:text-slate-400">{p.demo?.age ? `${p.demo.age}y` : "—"} / {p.demo?.sex || "—"}</p>
                    </div>
                  </div>
                ...
                <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity" onClick={(e) => { e.stopPropagation(); onDelete(idx); }}>
                  <div className="p-1.5 rounded-lg hover:bg-red-500/10 text-slate-400 dark:text-slate-500 hover:text-red-500 dark:hover:text-red-400 transition cursor-pointer">
